...
class Bd {
	...
	recuperarTodosRegistros(){
		...
		for(var i = 1; i <= id;i++){
			...
			//verifica se existe indices que foram pulados/removidos
			//netes casos vamos pular esses indices
			if(despesa == null){
				continue;
			}
	...
	pesquisar(despesa){
		...
		//filter não atua sobre o array original
		//se você quiser atualizar o valor precisa atribuir
...
function cadastrarDespesa(){
	...
	if(despesa.validarDados()){
		...
		document.getElementById('info').innerHTML = 'Despesa foi cadastrada com sucesso.';
		...
		document.getElementById('info').innerHTML = 'Erro na gravação, verifique se todos os campos estão preenchidos corretamente';
		document.getElementById('botao').className = 'btn btn-danger';
		document.getElementById('botao').innerHTML = 'Voltar e corrifir';
		$('#modalRegistraDespesa').modal('show');
	}
}
...
function carregaListaDespesa(despesas = Array(),filtro = false){
	...
	//percorrer o array depesas e listando elas
